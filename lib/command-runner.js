...
function executeCommand(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let child;
    try {
      // lgtm[js/command-line-injection] - command is sourced from user-controlled config, which
      // FlowSphere treats as privileged input (the operator owns the config file). Safe mode
      // (default) requires an absolute path to prevent PATH hijacking; shell: false ensures no
      // shell interpretation. Users explicitly opt into bare command names via allowUnsafeCommands.
      // lgtm[js/path-injection] - resolvedCwd is derived from user config (privileged input) via
      // path.resolve, which normalises the path. No filesystem write occurs; cwd only sets the
